...
func (s *SubstrateMainImpl) GetTwinPK(id uint32) ([]byte, error) {
	twin, err := s.Substrate.GetTwin(id)
	if err != nil {
		return nil, terr(err)
	}
...
func (s *SubstrateMainImpl) KVStoreSet(identity Identity, key string, value string) error {
...
func (s *SubstrateMainImpl) KVStoreDelete(identity Identity, key string) error {
...
func (s *SubstrateMainImpl) KVStoreGet(pk []byte, key string) (string, error) {
